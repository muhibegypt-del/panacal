...
| PGenerator's HDR10 output (HDR InfoFrame, 10-bit) | madTPG (`madtpg.py`), driven through madHcNet64.dll following madshi's madTPG.h (window placed fullscreen on the TV, HDR button and SMPTE 2086 metadata set by the run); the worker's 10-bit codes are passed through as 0..1 values. |
